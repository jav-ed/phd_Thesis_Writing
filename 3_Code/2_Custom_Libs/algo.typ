...
#let _get-code-line-data(
  ...
  // modified by javed -> take the max instead of the min
  // let real-text-width = calc.min(max-text-width, max-text-area-width)

  // let real-text-width = calc.max(max-text-width, max-text-area-width)
  let real-text-width = max-text-area-width
  ...
  for i in range(num-lines) {
    ...
    for j in range(
        calc.max(
          1, 
          calc.ceil(line-width/real-text-width)
        )
    ) {
...
#let _build-code-table(
  ...
  for (i, info) in flattened-line-data.enumerate() {
    ...
    // if the function of line number is activated
    // the goal is to only give actual new lines a own number/counter not long lines.
    // Long lines can be wrapped, however, wraping should not make them to new lines
    // The wrapping issue is important, to ensure the compataibiltiy with vscode
    if line-numbers {

      // in case a wrapped instance is found --> one long line --> do not change the line_Counter
      if is-wrapped {
        table-data.push([])
      } 
      else {

        // now an actual new line was found --> no long line, but simple a new line 
        table-data.push({
          set text(..line-number-styles)

          // the following two lines of performing code are modified/added by Javed
          // add the line_Counter_Custom
          str(line_Counter_Custom)

          // update the counter for the next full actual line (we do not give one single lines a new counter)
          line_Counter_Custom += 1
        })
      }
    }
...
#let code(
  ...
  // see: https://typst.app/docs/reference/meta/layout/
