...
def plot_function_analysis(expr_str, x_range=(-3, 3), num_points=1000):
    ...
    # Add tangent line at saddle point
    x_tangent = np.linspace(-4, 4, 500)
    y_tangent = np.zeros_like(x_tangent) + f(0)  # horizontal line at y=f(0)
    fig.add_trace(
        go.Scatter(
            x=x_tangent,
            y=y_tangent,
            mode='lines',
            name='Tangent at <br>Saddle Point',
    ...
    # Add zero-crossing annotation for first derivative
    fig.add_trace(
        go.Scatter(
            x=[0],
            y=[0],
    ...
    # Add zero-crossing annotation for second derivative
    fig.add_trace(
        go.Scatter(
            x=[0],
            y=[0],
...
if __name__ == "__main__":
    ...
    fig.write_image(output_fold/"2_Saddle_Point.svg")
