...
#glspl("swith") represent a novel approach in aviation fuel storage, integrating hydrogen tanks within the wing structure. This design aims to maximize space utilization while potentially contributing to load-bearing, distinguishing it from conventional fuel storage methods. Besides the scarcity of literature about #glspl("swith"), their unique characteristics present significant challenges. Unlike most structure-integrated approaches reviewed in literature, which predominantly focus on fuselage integration, #glspl("swith") involve wing integration. 
...
Second, even when assuming it would be known what to test, there remains a significant challenge in determining how to conduct these tests safely. Addressing these certification challenges is crucial for the successful implementation of #glspl("swith") in commercial aviation.
