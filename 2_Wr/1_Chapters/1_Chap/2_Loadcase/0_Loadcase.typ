...
To reduce training time, GPUs (Graphics Processing Units) @Kaur2023 @DeLuca2024 are essential processing resources. The widespread adoption of machine learning has driven advances in software frameworks that enable parallel training across multiple GPUs, further accelerating model development @Paszke2019 @tensorflow2015_whitepaper @Sergeev2018 @link_ray2024 @link_deepspeed2024 @link_accelerate2024.
While the training phase of deep learning models can be time-intensive, the subsequent inference time is substantially lower @butt2021development. Inference, which refers to generating model outputs, can often be performed in real-time. Additionally, depending on the neural network architecture, models trained on multiple GPUs may operate efficiently on a single GPU or even a CPU (Central Processing Unit). For models that fit within RAM (Random Access Memory), inference operations on standard CPUs typically execute orders of magnitude faster than high-fidelity #gls("fem") @Mittelstedt2021 @Mittelstedt2022 @Werkle2021a @Hahn2018 @Langtangen2016 @Langtangen2019 @Dhondt2004 or #gls("cfd") simulations.
...
According to @tab_29, vertical gust loading represents one of the most probable critical load cases, with a 100% probability of being dimensioning. Consequently, this thesis identifies vertical gust loading combined with internal pressure as the crucial load case for #glspl("swith"). As elaborated in @chap_1_0_3, the internal pressure significantly influences the weight, costs, and safety characteristics of the pressure vessel.
