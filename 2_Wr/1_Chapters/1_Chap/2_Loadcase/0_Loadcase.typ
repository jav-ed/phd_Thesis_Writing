...
In this subsection the goal is to find loadcases that ...


In this section, it will first be explained why the identification of critical load cases is significant and how K2H2 differs from conventional load cases. Subsequently, the methodological approach for K2H2 will be substantiated and outlined.

The definition of critical load cases for structure-integrated hydrogen tanks, designed as integral parts of light aircraft wings, presents a challenging task. The tanks are, on one hand, integrated into the wing structure and thus bear structural loads. On the other hand, the tanks are under high pressure. The internal pressure particularly leads to increased complexity, as it may now be necessary to test each conventional load case both without pressure and across an entire pressure range.
Even for conventional light aircraft, the identification of critical load cases poses a significant challenge. There exists a vast array of possible load cases. Examining each individual load case experimentally is neither economically viable nor environmentally feasible in practice. Simulative investigations present the alternative. However, depending on the required accuracy, a single calculation of aerodynamic loads may take months. Even if it were possible to write very fast and accurate simulation software, numerous experienced programmers, fluid mechanics engineers, structural mechanics engineers, optimization specialists, and others would be required.
Given the assumption of having such solvers, substantial hardware resources and qualified personnel capable of operating the software would be necessary. Once results are obtained, all of these must be interpreted and categorized. It is crucial to filter out from these numerous results those that represent critical load cases. Therefore, according to the current state of technology, it is not possible to calculate all specified load cases with high accuracy in real-time, compare them with each other, and obtain the critical ones as a final result.
Within the framework of K2H2, it is neither temporally nor financially viable to model, calculate, evaluate all individual load cases, and then finally arrive at a few selected candidates. However, it should be unequivocally stated that the considered load cases are essential for the design of an aircraft's airworthiness and safety.


In order to identify potentially realistic critical load cases for K2H2, it was initially relied upon the critical load cases provided by the former company MMM. The author derives the critical load cases presented in Table 26 not directly from MMM, but from the lecture course "Design of Commercial Aircraft II," delivered by Dr. Heinze at TU Braunschweig. The company MMM had reportedly simulated various load cases and then statistically evaluated the probability of the tested load cases being dimensioning. At this point, gratitude is extended to Dr. Heinze for providing this information, especially considering that comparable information typically represents protected proprietary knowledge of very large companies. In other words, external scientists do not have access to these documents.
...
Here's the academic translation:
In K2H2, it is not possible to experimentally replicate all identified load cases. Rather, due to cost and time constraints, it is necessary to select one or at maximum two load cases. Simulative modeling of the critical load cases from Table 26 is also not readily feasible. Depending on accuracy requirements, high-quality and time-intensive CFD calculations, fluid-structure coupling, multi-body simulation, crash analysis, or a combination thereof must be computed.
Given that vertical gust loading is considered one of the most significant loads, it warrants closer examination. The loads can be derived from a Vn diagram and can be practically implemented into existing methods. The Vn diagram itself prescribes an entire range of possible load cases. However, the critical cases among these have already been identified in the literature. Furthermore, the Vn diagram is valid for all common types of light aircraft. Thus, there exists the possibility that the knowledge gained through K2H2 may also be applicable to other hydrogen-powered light aircraft.
