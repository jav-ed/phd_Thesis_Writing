...
The global goal is to answer important questions for the experimental trial. Among other things, how many #gl_("lie") (Load Introduction Elements) should be used, where should they be placed, and what forces should be applied. If a suitable objective function is found, this question could be answered directly with a #gl_("fem") coupling. In other words, it is entirely possible for the optimizer to have access to the #gl_("fem") solver. In this case, the optimizer could obtain important structural information through a #gl_("fem") calculation while exploring the unknown territory. Since optimization is generally an iterative process, a #gl_("fem") solution would be needed for each individual function evaluation. However, within a single iteration step, significantly more function evaluations might be required. This can be understood particularly easily using an evolutionary algorithm as an example. In each individual iteration step, multiple parameter combinations for the design variables are examined. For each individual parameter combination, at least one function evaluation would be required. Depending on how exactly the objective function is defined, multiple function evaluations might also be required for a single parameter combination. The latter would be possible if two #gl_("fem") calculations were necessary. If a function evaluation is cost-effective in terms of time and hardware, this factor can be neglected.

However, with a three-dimensional structural model of an aircraft, the summation of multiple function calls of a #gl_("fem") calculation might be unacceptably high under certain circumstances. This is the first reason why an alternative to the #gl_("fem") calculation in the optimization loop was sought. The other reason is that initially, no clear objective function was known for the present problem. This itself was found iteratively. If each individual optimization were associated with about a month of calculation time, undesirable limitations would occur. When comparing the beam model with the #gl_("fem") calculation, the following is clear: The beam model has lower accuracy reproduction, but is faster in calculation than the high-quality #gl_("fem") model.
