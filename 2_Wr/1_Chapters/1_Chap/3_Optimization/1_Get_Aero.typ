...
To convert from a coefficient distribution to a force, let's examine @eq_9[Equations] and @eq_10[].
In @eq_9, the variables represent $F_a$ for a point load, $rho$ for density, $u²$ for velocity squared, $C_l$ for lift coefficient, $c$ for chord length, and $l$ for length.
Furthermore, $q_a$ from @eq_10 represents a force distributed over a length, or a line load.

// dot is used because it makes the distinction between the variables much easier -> easier than just adding some space.med
$ F_a = rho/2 dot u² dot C_l  dot c dot l space.quad  [#unit("N")] $ <eq_9>
$ q_a = rho/2 dot u² dot C_l  dot c  space.quad [#unit("N/mm")] $ <eq_10>
...
A visual representation of the difference between the distributed load and point load is given in @fig_17 on the right side. 
If the uniform line load is to be formed into a point load, it would act at the center as shown in @fig_17.
Furthermore, @fig_17 shows that the example wing has a mesh consisting of individual elements. 
The result that APAME provides is a line load that follows @eq_10.
However, to work with kmeans++ @Arthur2006, discrete point data is needed.
Therefore, as shown in @fig_17, it is necessary to move from the representation of @eq_10 to the representation of @eq_9.
...
The following will show in detail using @fig_18 how a line load can be converted to a point load suitable for kmeans++.
First, it is known that according to @fig_17, the mesh consists of many individual elements and each element has its own line load.
...
To convert from a uniform line load $q_a$ to a point load $F_a$, @eq_11 can be used.
Here, $l$ is a side length. 
Since we obtained $q_a$ according to @eq_10, the length in chord direction has already been used. 
...
Mathematically, this can also be expressed as the L2-norm as shown in @eq_12.
...
Therefore, @eq_13 gives the number of partial forces into which a resultant point load should be subdivided.
The individual partial point loads can be calculated with @eq_14. 
...
$ n_F = l_i 20/100  $ <eq_13>

$ F_(a,i,j) = (q_(a,i) dot l_i )/n_F  $ <eq_14>
...
This behavior is shown in @eq_15, the lift coefficient which is valid for a strip $i$ is denoted as $C_(op("lc"),i$ and is obtained through the integration of the lift coefficient $C_l$ over the boundaries le and te.
...
$ C_(op("lc"),i) = integral_(op("le")) ^(op("te"))  C_l $ <eq_15>
...
To obtain the force from this said resultant lift coefficient, @eq_9 can be used.
