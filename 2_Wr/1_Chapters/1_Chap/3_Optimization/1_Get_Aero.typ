...
To convert from a coefficient distribution to a force, let's examine @eq_75[Equations] and @eq_76[].
In @eq_75, the variables represent $F_a$ for a point load, $rho$ for density, $u²$ for velocity squared, $C_l$ for lift coefficient, $c$ for chord length, and $l$ for length.
Furthermore, $q_a$ from @eq_76 represents a force distributed over a length, or a line load.

// dot is used because it makes the distinction between the variables much easier -> easier than just adding some space.med
$ F_a = rho/2 dot u² dot C_l  dot c dot l space.quad  [#unit("N")] $ <eq_75>
$ q_a = rho/2 dot u² dot C_l  dot c  space.quad [#unit("N/mm")] $ <eq_76>
...
The result that APAME provides is a line load that follows @eq_76.
However, to work with kmeans++ @Arthur2006, discrete point data is needed.
Therefore, as shown in @fig_67, it is necessary to move from the representation of @eq_76 to the representation of @eq_75.
...
To convert from a uniform line load $q_a$ to a point load $F_a$, @eq_77 can be used.
Here, $l$ is a side length. 
Since we obtained $q_a$ according to @eq_76, the length in chord direction has already been used. 
...
Mathematically, this can also be expressed as the L2-norm as shown in @eq_78.
...
$ l = sqrt((x_(i,r) - x_(i,l))² + (y_(i,r) - y_(i,l))² + (z_(i,r) - z_(i,l))²) $ <eq_78>

// ------------------------------- point load ------------------------------- //
Thus, with @eq_77 and @eq_78, a line load can be converted to a point load.
...
According to @eq_79, at a length of $100 #unit("mm")$, a distributed load [N/mm] is approximated by 20 partial point loads [N].
Therefore, @eq_79 gives the number of partial forces into which a resultant point load should be subdivided.
The individual partial point loads can be calculated with @eq_80. 
...
$ n_F = l_i 20/100  $ <eq_79>

$ F_(a,i,j) = (q_(a,i) dot l_i )/n_F  $ <eq_80>
...
This behavior is shown in @eq_81, the lift coefficient which is valid for a strip $i$ is denoted as $C_(op("lc"),i$ and is obtained through the integration of the lift coefficient $C_l$ over the boundaries le and te.
...
$ C_(op("lc"),i) = integral_(op("le")) ^(op("te"))  C_l $ <eq_81>
...
To obtain the force from this said resultant lift coefficient, @eq_75 can be used.
