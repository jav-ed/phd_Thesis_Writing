...
This can be represented as @eq_82[Equations], @eq_83[] and @eq_84[] @Gross2021b.
...
For a calculation in three dimensions, @eq_85[Equations] - @eq_90[] can be used.
Here, the force balance in the third dimension is added through @eq_87 and the moment balances in the two remaining rotation axes in @eq_88 and @eq_89.

$ sum F_(i,x) = 0 $     <eq_85> 
$ sum F_(i,y) = 0 $     <eq_86>
$ sum F_(i,z) = 0 $     <eq_87>
$ sum M_(i,x)^(A) = 0 $  <eq_88>
$ sum M_(i,y)^(A) = 0 $ <eq_89>
...
Important relationships in calculating section force distributions are given by @eq_91[Equations], @eq_92[] and @eq_93[] @Gross2021b.
Here, @eq_91 states that the derivative of the shear force $Q space.thin[#unit("N")]$ with respect to the spatial coordinate $x$ equals the negative distributed load $q space.thin [op("N/mm")]$.
@eq_92 states that the derivative of the bending moment $M space.thin [op("N mm")]$ with respect to the spatial coordinate $x$ equals the shear force $Q space.thin[#unit("N")]$.
Furthermore, @eq_93 shows the relationship between @eq_91 and @eq_92.

$ (dif Q) / (dif x) = -q  $    <eq_91>
$ (dif M )/ (dif x) = Q  $     <eq_92>
$ (dif² M) / (dif² x) = -q  $   <eq_93>

When @eq_91[Equations] - @eq_93[] are inverted, equations @eq_94[], @eq_95[] and @eq_96[] are obtained.

$ Q = - integral q dif x   $    <eq_94>
$ M = integral Q   dif x $     <eq_95>
$ M   = - integral integral q dif x  $   <eq_96>

All equations from @eq_82[] to @eq_96[] can be used to calculate and plot normal force and shear force distributions, as well as bending moment distributions, analytically and by hand.
