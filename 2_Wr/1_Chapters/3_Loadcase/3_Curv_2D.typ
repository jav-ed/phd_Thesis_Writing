...
This distinguishes it from method 2, where the gradients $u'$ and $u''$ are derived from the averaged displacement $overline(u)$. The fundamental difference lies in the gradient computation, which follows the methodology of method 1 but applies averaging at each derivative stage. The mathematical formulation of this approach is outlined in @eq_55.
...
// ---------------------------- shape information --------------------------- //
The three approaches proposed in @eq_53 to @eq_55 aim to transform the three-dimensional curvature representation into a one-dimensional representation. More specifically, this transformation converts the curvature matrix from a dimension of $bold(kappa)^(n times 6)$ to either $bold(kappa)^(n_y times 4)$ or $bold(kappa)^(n_y times 2)$, where $n_y$ defines the total number of discretization points along the span direction (y-axis).
The original $bold(kappa)^(n times 6)$ matrix structure comprises two main parts: the first three columns contain local coordinate information for each spatial axis ($x,y,z$), while the remaining three columns contain the curvature components in their corresponding directions. When the curvature components for a numerically constant span section $y_i$ are averaged across the chord (x) and vertical (z) axes, the matrix is reduced to $bold(kappa)^(n_y times 4)$. In this form, the first column specifies the position along the span, and the remaining three columns contain the curvature components in the three spatial directions.
...
This $bold(kappa)^(n_y times 2)$ representation is the desired outcome as it enables straightforward interpretation through two-dimensional plotting and provides the advantages elaborated at the beginning of this section.

// ----------------------------- examples calcs ----------------------------- //
Although three seemingly distinct approaches for curvature averaging were presented, analytical analysis reveals that only two fundamentally different methods exist. 
This can be validated through the application of trigonometric terms in @eq_53 to @eq_55.
For clarity, this analysis is demonstrated using a simplified case: a single spanwise section $y_i$, one dimension $j$, and two terms for the sum in the mean curvature equations. The trigonometric terms are formulated in @eq_56, with their first derivatives given in @eq_57, and second-order derivatives in @eq_58.
