...
// -------------------------------------------------------------------------- //
Initial investigations regarding external loads on pressure cylinders were presented in @chap_2_3. The rationale for studying external dynamic loads on pressure vessels stems from the essential standards detailed in @chap_1_5. Additional justification for considering dynamic loads in aircraft applications was substantiated in @chap_3_0. Since #glspl("swith") integrate both pressure vessel and aircraft characteristics, the significance of dynamic loading extends naturally to #gls("swith") applications.
Furthermore, these investigations could potentially yield insights relevant to both dynamic and static testing scenarios.
This pattern is particularly noteworthy although the literature predominantly focuses on external dynamic loading of pressure vessels rather than external static loading.
After examining the dynamic loading aspects, the analysis conducted in @chap_3_0 designated the vertical gust load as the critical static test load for this thesis. 
...
Air-filled pipes, following @eq_23, demonstrate a substantial increase in maximum curvature of $1.05 space upright("m")^(-1)$, while according to @eq_24, water-filled pipes yield a more modest increase of $0.3  space upright("m")^(-1)$.
...
The maximum curvature demonstrates more pronounced changes, with a difference of $1.25 space upright("m")^(-1)$ at $50 "bar"$ (@eq_16) that reduces substantially to $0.5 space upright("m")^(-1)$ at $150 "bar"$ (@eq_17).
...
This discrepancy could culminate in the certification of structures with inadequate safety margins for their intended operational use with hydrogen.
...
// --------------------------------- summary -------------------------------- //
#summary_([
In summary, the study of fill media effects on pressurized structures has revealed several critical findings.
Under static external loading, both the internal pressure and the choice of media have a substantial influence on the structural behavior.
Water-filled pipes consistently exhibited superior performance compared to air-filled counterparts, achieving higher maximum bending moments and greater curvature tolerance across all tested pressure ranges. 
