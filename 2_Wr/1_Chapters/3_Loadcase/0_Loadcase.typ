...
The evaluation of these standards indicated varying flexibility regarding tank filling agents. While some testing protocols permitted the substitution of hydrogen with alternative filling agents, others mandated the use of hydrogen. For practical applications involving hydrogen, a thorough understanding of its properties is vital. These properties were extensively elaborated in @chap_2. 


In @chap_2_2, it was demonstrated that while hydrogen can be utilized in experimental settings, its direct application to long tubes that are under high pressure, as typically required for #glspl("swith"), presents significant challenges.
To safely conduct experimental investigations with high-pressure hydrogen tubes matching the dimensions of an aircraft wing, a long-term strategy was proposed.
This approach advocates for a gradual progression of simulations and experiments, starting with smaller tube lengths and lower pressure values, systematically increasing both parameters as safety protocols are validated. 
...
The primary objective of this chapter is to determine whether hydrogen can be substituted with an alternative filling agent without substantially altering the structural properties that would invalidate test outcomes. 
To address this question, critical load cases will be examined in @chap_3_0. 
@chap_3_1 offers experimental studies comparing the behavior of liquid and gas-filled pressure vessels. Based on these findings, a critical curvature is introduced as a key metric for assessing filling media impact.
...
The inherent complexity arises from the dual nature of #glspl("swith"): the tubes simultaneously function as integral elements of the wing structure, bearing aerodynamic loads while also containing high internal pressures.
This duality necessitates that each conventional load case be evaluated across a spectrum of pressure conditions, from unpressurized states through various pressure levels, substantially expanding the testing matrix.
Identifying critical load cases is challenging even for conventional general aviation aircraft, given the extensive range of scenarios prescribed in certification standards @EASA_CS_23 @EASA_CS_25.
A comprehensive experimental investigation encompassing all potential load cases proves prohibitive from both economic and environmental sustainability perspectives.

While numerical approaches offer a potential alternative pathway, these methods introduce their own set of obstacles. Single aerodynamic load calculations may involve months of processing time, with the computational load varying significantly based on accuracy requirements @Yang2021a @HernandezAguirre2022 @Kochkov2021 @Blazek2015 @Fu2020.
// --------------------- drawbacks of regular simulation -------------------- //
According to @Yang2021a, Direct Numerical Simulation (DNS) consumes 100 times more computing resources than wall-resolved large-eddy simulation (WRLES) for Reynolds numbers ranging from $10^7$ to $10^9$ @Sigloch2022 @Schlichting2017. 
...
The practical implications of these computational burdens are substantial. 
A DNS study on Argonne's Mira supercomputer is an illustration of these resource challenges. 
The investigation utilized 25 million core hours, while comparable WMLES cases averaged only 150,000 core hours on the same system, revealing DNS to be approximately 150 times more compute intensive than WMLES @Fu2020. Direct numerical simulations face crucial scalability barriers. Despite their foundation in fundamental physics laws and equations of motion, these simulations remain technically infeasible at scales necessary for critical applications such as climate and weather modeling @Kochkov2021.
While simplified aerodynamic simulations can be achieved by omitting friction effects and introducing other simplifications @Barba2019 @Drela1989 @aerosandbox_phd_thesis @Verdugo2022 @Liu2022a @Wild2022, such compromises may not meet the stringent standards set by certification authorities.
...
The development of surrogate models through these approaches requires substantial training data. For high-fidelity simulations, this training data necessarily comprises outputs from existing high-fidelity numerical models. When sufficient training data is available, numerous modeling methods can be implemented @Charbuty2021 @Palimkar2021 @Cristianini2000 @Murty2016 @Suyal2022 @Dumitrescu2022 @Itoo2020 @Ahmad2020 @Li2023c @vaswani2017attention @Sastrawan2022 @You2022 @Qu2024 @Robinson2022. While model training demands notable computational resources, the inherent structure of machine learning algorithms, especially deep learning architectures, enables efficient parallelization of these calculations.
...
The efficiency gains extended beyond raw execution speed. The machine learning approach required 132 times fewer simulation evaluations and reduced total core-hours by a factor of 144 compared to traditional physics simulations. Through iterative retraining, potential further reductions of 330-550 times fewer simulation evaluations were identified. Their more complex test case, the IsoDAR cyclotron, demonstrated even more improvements, with the neural network executing $cal(O)(10^7)$ faster than the original physics simulation. 
These improvements maintained good accuracy compared to the underlying physics simulations, while the neural network training consumed only 10 minutes on a laptop.
Comparable efficiency improvements in #gls("cfd") have been documented, with speedup factors between 40 and 80 @Kochkov2021.


Having mentioned some advantages of machine learning models as surrogates, two important factors deserve emphasis. First, the cited speedup in calculations refers specifically to the inference phase, not the training phase. Second, developing machine learning models demands substantial input data. For high-fidelity simulations, this means conducting computationally expensive simulations to generate training data. Therefore, if pre-trained models are not available, the initial high-fidelity numerical simulations remain necessary. Consequently, all obstacles associated with high-fidelity simulations persist during the data generation stage for machine learning models.

Given these limitations in both traditional simulation approaches and machine learning alternatives, establishing comprehensive simulation capabilities presents considerable resource difficulties beyond just computational power. Even with advanced computing methods, successful analysis relies on expertise in several technical domains. These include #gls("cfd") for accurate aerodynamic load prediction, structural mechanics for understanding load distribution and material behavior, and multi-disciplinary strategies @Wang2021 @Meng2022 @Mader2020 @Meng2021 @Li2019a. 
The complexity is further increased by the potential need for multi-objective optimization techniques @Tian2021 @Li2023d @Ridha2021 to properly define requirements as optimization problems and manage the vast solution space.
...
Yet, it is important to note that the central hurdle lies not in solving any single physics or coupled problem, but rather in the systematic evaluation of numerous load cases, each potentially requiring different combinations of physical models and solver configurations.
Moreover, the interpretation of such multidimensional results relies on sophisticated post-processing methodologies to identify truly critical load cases, as the interaction between different physical phenomena may lead to non-obvious failure modes. 

To give some perspective on why load case simulations for #gls("swith") are complex and multidisciplinary, @tab_12 and @tab_13 provide relevant examples. 
The diverse expertise involved becomes evident when comparing different test scenarios. For instance, simulating a gunfire test demands inherently different capabilities than those needed for a pneumatic cycle test or a glass transition temperature test.
The reasons for inclusion of pressurized cylinder tests for #glspl("swith") were elaborated in @chap_1_5. Since this thesis focuses on #glspl("swith") using #gls("cgh2", long:true), it is reasonable to anticipate that some or all tests required for pressurized cylinders may also be applicable to high-pressure vessels when integrated as load-bearing components within an aircraft's wing. 
Additionally, aircraft standards such as @EASA_CS_23 and @EASA_CS_25 outline numerous load cases that must be taken into account as well.


// --------------------------- general load cases --------------------------- //
The following description of general loads that occur during flight and on ground operations draws primarily from @heinze2023aircraft. Aerodynamic forces induced during flight operations, specifically the pressure differentials that generate lift and negative lift forces, form a primary load category. These aerodynamic loads manifest predominantly during dynamic flight maneuvers. 
...
the loads can be gradually increased from calibration to limit, ultimate, and finally rupture testing. 
...
The implications of pressure selection can be analyzed from multiple perspectives. From an economic standpoint, higher pressure values enable increased hydrogen storage capacity. From a structural perspective, elevated pressure values intensify the loads on both the pressure vessel and the wing structure due to the structural integration. The safety implications, as elaborated in @chap_2_1 and @chap_2_2, indicate that higher pressure values increase the risk of self-ignition in case of leakage. 
From a practical testing perspective, as explained in @chap_1_5 with reference to @tab_14, experimental structural validation tests are constrained by maximum allowable pressure limits.
Furthermore, @chap_2_2 established a systematic pathway for conducting structural validation tests of #glspl("swith"). 
Specifying appropriate internal pressure values when using #gls("cgh2") as the filling agent presents major challenges, with substantial implications for the safety of the #gls("swith"), the test environment, and human operators. 
...
// ------------------------------- v-n-diagram ------------------------------ //
Having demonstrated the critical nature of vertical gust loading for #glspl("swith"), it is essential to understand how these loads are determined in aircraft design. Vertical gust loads can be systematically obtained and analyzed through V-n diagrams, also known as flight envelopes or V-g diagrams. 
The flight envelope characterizes the relationship between load factors and airspeed within which an aircraft is designed to operate @Gudmundsson2014 @Rossow_2014. It encompasses two distinct categories of loads: maneuver loads and gust loads @Rossow_2014. An illustrative example of a V-n diagram conforming to #gls("easa") #gls("cs")-23 @EASA_CS_23 specifications for the normal category is depicted in @fig_28.
...
The vertical axis of @fig_28 represents the load factor $n$, which according to @Rossow_2014 can be described by @eq_11. The variable $P$ designates a force typically acting against the gravitational force $G$, where $m$ and $g$ specify the mass and gravitational constant, respectively.

$ n = P/G = P / (m space.thin g) $<eq_11>

The load factor is dimensionless and corresponds to the safe load or limit load @Rossow_2014. The limit load is defined as the maximum load expected during regular operating lifetime that does not induce plastic deformation or permanent deflections @Rossow_2014. No structural failure should occur before reaching the ultimate load, which is given by @eq_12, where a standard safety factor of 1.5 is applied:

$ "ultimate load" = 1.5 times "limit load" $<eq_12>

The safety factor is intended to account for variations in loads and material properties, as well as potential inaccuracies in calculation methods. Additionally, it helps to prevent catastrophic structural failure if operating limits are marginally exceeded @Rossow_2014.
