...
In the previous @chap_4_2, fundamental principles of the beam modeling were presented, including elaborations on internal force distributions, which will be relevant for optimization as shown in @chap_4_4 and @chap_4_5_0 to @chap_4_5_3. 
...
// ---------------------------- global and local ---------------------------- //
In addition to multimodality, @fig_72 serves to highlight another core concept in optimization: the distinction between local and global extrema. The function depicted in @fig_72 features distinct points of absolute maximum and minimum values. Within the entire considered function space, the point with the highest value is defined as the global maximum, while the point with the lowest value defined as the global minimum.
...
From an economic perspective, pursuing global minima may not always represent the optimal strategy. The feasibility of finding such solutions depends on multiple factors, including the nature of the optimization problem, computational resources, and the selected optimization method. Two primary classes of optimization methods exist: gradient-based and gradient-free approaches, each with distinct advantages and limitations.
...
This consideration becomes especially significant when the objective function $J(x)$ operates on a vector of input variables, extending these concepts to higher-dimensional spaces.
...
A fundamental principle in optimization is to maintain parsimony in the selection of design variables, as each additional variable can increase the dimensionality of the design space notably. This increased dimensionality typically yields higher computational costs and more complex solution spaces.
...
// -------------------------------------------------------------------------- //
A high number of design variables is warranted only when both sufficient computing resources are available and the modeling accuracy demands such complexity. In such cases, sensitivity analysis serves as a structured approach to evaluate the significance of each design variable.
...
Others, in contrast, may resist complete mathematical formulation due to their inherent qualitative nature. 
In such cases, developing appropriate numerical representations may cost significant effort. The obstacle lies in attempting to capture the essential characteristics of these disciplines while maintaining computational feasibility.
...
When a high number of design variables is deemed necessary for accuracy requirements, sensitivity analysis enables the identification and removal of less influential variables. 
This reduction in design variables simplifies the optimization problem, leading to decreased computational requirements and hardware demands. 
...
Several aspects can impede successful convergence. The selected optimization algorithm may prove unsuitable for the specific problem characteristics, or the solution path may display oscillatory behavior. 
...
While gradient-based optimizers tend to converge to local minima, such solutions often return sufficient improvements over initial conditions. Therefore, local minima warrant consideration rather than immediate rejection.
...
In gradient-based optimization, the accuracy of these gradients substantially influences the optimization trajectory. Unreliable gradient information may lead to either non-convergence or physically unrealistic outcomes. In cases where gradient calculation proves impossible, gradient-based methods become inapplicable.

Gradient-free optimizers offer distinct advantages, especially in their enhanced capability to identify global optima and their compatibility with all variable types, as they operate independently of gradient information.
