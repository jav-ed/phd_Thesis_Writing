...
As laid out in @chap_3_4, selecting an appropriate #gls("lie") or a combination of them warrants additional information.
...
The following section explores various approaches to this discretization hurdle, evaluating both traditional regression-based methods and modern unsupervised machine learning techniques. Through systematic comparison of these approaches, a preferred method is identified based on reasoned analysis.
...
When applying these principles to polynomial regression models, distinct behavioral patterns emerge. L2-norm optimization of a fourth-degree polynomial model maintains non-zero values across all five parameters, reflecting effective interpolation within the training domain. 
...
This stark contrast, combined with the demonstrated computational capabilities of modern hardware, indicates that computational constraints are not limiting factors in the selection of a load approximation method. Instead, accuracy requirements emerge as the primary criterion in selecting appropriate approximation methods.
In evaluating accuracy optimization, the choice between L1 and L2 norms becomes significant. The L2-norm yields advantages for interpolation tasks, particularly when rapid convergence during optimization is prioritized. This characteristic proves beneficial for models requiring flexibility within the training domain. Conversely, the L1-norm exhibits an inherent capacity to nullify superfluous model parameters during optimization, effectively setting them to zero. This property renders L1-norm optimization especially suitable for extrapolation tasks.
Given that physical reality demands both interpolation and extrapolation capabilities, the selection of an appropriate norm warrants careful consideration. 
...
The formal optimization problem is expressed in @eq_75, following the notation established in @Frochte2020. In this formulation, $d$ indicates the distance function while $x_j$ denotes individual data points. The cluster mean for cluster $i$ is expressed by $mu_i$, and $k$ indicates the total number of available centroids.
...
A comprehensive review of these methods extends beyond this work's scope. Interested readers may find detailed elaborations in @Ikotun2023 and through the supplementary resources available at @link_Cluster_Meth. @Ikotun2023 further explores recent developments in k-means and its expanding applications.
...
The centroid determination process employs geometric averaging, which generates characteristic points for each cluster. However, these averaged values prove inadequate for accurately reflecting the resulting loads in #glspl("ld"). Applying averaged forces would fail to properly account for the cumulative effect of all partial loads within each cluster, leading to physically incorrect force representations.
...
The diamond-shaped markers in the top plot indicate the two-dimensional positions of each centroid. The adjacent numerical values correspond to the force magnitudes computed by k-means++. During its iterative process, k-means++ uses mean values for updates until convergence. At convergence, these centroid values represent the mean forces within their respective clusters.
...
The cluster regions, visualized as color-coded areas in the upper portion of @fig_66, provide a valid theoretical basis for specifying the dimensions of an individual #glspl("lie"). 
...
To illustrate this limitation, consider a pressure testing scenario where internal pressure mandates incremental increase from $10 #unit("MPa")$ to $60 #unit("MPa")$. During this process, the ability to visually detect potential structural anomalies or damage becomes critical. Excessive #glspl("lie") coverage would substantially compromise this essential monitoring capability.
