...
This section reports on research outcomes focused on utilizing numerical optimization to discretize aerodynamic loads. The optimization framework provides solutions to crucial engineering questions, including the determination of optimal #gls("lie", long:true) quantities, as well as their force magnitudes, lengths, and precise positioning.
The IMA Materialforschung und Anwendungstechnik GmbH Dresden was consulted to incorporate real-world engineering concerns relevant for potential #gls("swith", long:true) certification processes. 
Through systematic analysis, these practical requirements were translated into mathematical descriptions for computational optimization. 
The optimization formulations are presented, explained, solved, and interpreted with respect to their performance.

The investigations are structured into multiple optimization cases, beginning with Case A as the fundamental approach in @chap_4_5_0. 
Each subsequent case, described in @chap_4_5_1 to @chap_4_5_3, incorporates additional constraints and engineering considerations that emerged through iterative consultations with structural testing experts at IMA Dresden. These considerations reflect real-world requirements, ensuring the optimization framework's practical applicability.  
The progressive refinement of constraints through expert consultation serves multiple purposes. One of these purposes is bridging the gap between theoretical optimization and practical test implementation requirements for potential #gls("swith") certification.
The developed optimization framework demonstrates adaptability to varying experimental testing demands. Furthermore, it reveals how distinct optimization problem definitions can effectively address different, yet equally valid, real-world requirements. 
This flexibility proves valuable for both certification authorities and manufacturers, who may encounter diverse technical specifications and constraints.

It shall be highlighted that although the cases are presented in order of increasing complexity, this hierarchy does not imply the superiority of later cases. Instead, each optimization case represents a distinct definition tailored to specific scenarios. The presentation of multiple optimization cases illustrates how the mathematical framework and implemented tools can be adapted to address diverse project-specific demands while maintaining a consistent optimization methodology.
...
The constraints that follow in the #emp_[subject to] portion are formatted 
with design variables on the left side and constant terms on the right side of the inequality ($bold(x)_d <= $ constant parameters).
Consequently, the positions $x_i$ are not design variables but rather define the centers of the centroids, determined through k-means++ clustering as detailed in @chap_4_0 and @chap_4_1. 
These centroid locations physically correspond to the centers of the #glspl("lie").

The first constraint ensures that the #glspl("lie") remain within the considered beam length, where ha denotes the half-span length. 
...
This approach ensures global accessibility of the methodology, facilitating its adoption and further development by the scientific community. 
Following this principle, SciPy @Virtanen2020a @RalfGommers2024 was chosen to serve as the core optimization engine.
According to #link("https://scholar.google.com/")[scholar.google.com], SciPy has been cited $num("33859")$ times through @Virtanen2020a, demonstrating its extensive application across scientific domains. Furthermore, SciPy is widely recognized for providing powerful, computationally optimized, and reliable algorithms.
...
The @eq_110 to @eq_112 express the linear constraints through lower $bold(l)$ and upper bounds $bold(r)$, described as vectors with dimensions corresponding to the number of constraints $m$. The design variable coefficient matrix $bold(A)_c$, which assigns weights to each design variable, maintains dimensions of $m$ constraints by $n$ design variables.
...
// ------------------------------- beam model ------------------------------- //
@fig_80 presents the fundamental beam model configuration and its analytical solution, which serves as the reference for optimization. 
The figure consists of four subfigures arranged in rows, each providing essential information for the analysis.
In the first row, the beam schematic illustrates the support configuration and loading conditions. 
...
While APAME @Filkovic generates numerous discrete load values, as detailed in @chap_4_1, displaying individual numerical values would compromise the figure's clarity due to the high density of data points. 
Instead, a red continuous distribution curve is employed to characterize the true aerodynamic load distribution, providing a clear visual representation of the loading pattern and maintaining figure legibility.

By synthesizing the outlined information from the first row of @fig_80, a surrogate model that serves as an efficient preliminary wing modeling approach is established.
...
The span length selection significantly influences both the optimization process and its computational requirements. 
The available design space for an optimization expands proportionally with span length, as longer spans create more possible combinations for design variable values. For instance, a normalized span length of $1 "mm"$ creates a relatively confined design space between 0 and 1, whereas actual aircraft spans can be substantially larger. As referenced in @chap_1_5, large aircraft can have span lengths of approximately $80 "m "$ or $num("80000") "mm"$, creating a vast design space that demands increased computational resources.
Current #gls("swith") development focuses primarily on smaller aircraft, as evidenced by APUS's efforts @APUS_0, mentioned in @chap_0_7. Given this context and considering computational efficiency, this study employs a beam length corresponding to smaller aircraft dimensions. Specifically, the length was selected to match the half-span of the APUS i-2 @APUS_1, leveraging the assumption of vertical axis symmetry in wing design. 
However, the methodology's formulation remains scale-independent, making it applicable across various aircraft categories from small unmanned aerial vehicles to large commercial aircraft.
// -------------------------------------------------------------------------- //

Prior to presenting the optimization results, it is important to highlight the unit conventions employed in this analysis. 
The calculations for the optimizations were performed using Newton-millimeters (Nmm) to enhance numerical precision. 
In contrast to @fig_80, which shows the beam model results in Nmm, subsequent visualizations of the optimization results are converted to Newton-meters (Nm) to conform to common engineering practice.
The results of optimization case A with four and eight #glspl("lie") are depicted in @fig_81 and @fig_82, respectively. Both figures maintain consistent visualization conventions across four rows:
...
// ----------------------------- interpretation ----------------------------- //
Analysis of the optimization results reveals that the computed bending moment distributions closely match the reference cases, showing minimal visible deviation. Although the objective function specifically targeted bending moment behavior, the shear force distributions also demonstrate close alignment between the optimized and reference cases. 

The approximation accuracy notably improves with an increasing number of #glspl("lie"), as evidenced by comparing @fig_81 and @fig_82.
This improvement is further quantified by the objective function values presented in @fig_83, which reveal a clear inverse relationship between the number of #glspl("lie") and the objective function value.
The second row of @fig_83 illustrates the computational efficiency of the approach by depicting the number of iterations required for convergence.
The relatively low iteration count reflects the effectiveness of the method. This efficiency can be reasonably attributed to the strategic use of k-means++ clustering for determining #gls("lie") center positions.
This approach significantly reduces the number of design variables compared to implementations where #gls("lie") positions are treated as additional optimization parameters. For a half-span length ranging from $gt.approx 5000/2 "mm"$ to $frac(approx #num("80000"),2) "mm"$, this reduction substantially limits the optimization design space, leading to faster convergence times. 
The significance of this computational advantage becomes particularly evident in subsequent optimization cases.
