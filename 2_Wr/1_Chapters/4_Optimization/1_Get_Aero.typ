...
The transformation process holds particular significance for the k-means++ implementation and demonstrates advantageous implications for the subsequent optimization framework.

// -------------------------------------------------------------------------- //
Prior research at TUD Dresden University of Technology established foundational work in this domain, particularly through the development of a computational framework that integrates structural and aerodynamic analyses using #gls("fem", long:true). This framework has been made publicly available under the GNU General Public License and can be accessed at @link_Fipps.
It employs APAME @Filkovic as its aerodynamic solver, utilizing a three-dimensional frictionless panel method for load calculations.
While APAME does not target high-fidelity aerodynamic analysis, it provides an effective balance between computational efficiency and accuracy in predicting lift distribution. 
The solver processes several key input parameters, including flight velocity, density, Mach number, and geometric characteristics, to generate spanwise lift distributions. 
Through fluid-structure coupling, these aerodynamic loads are mapped from the aerodynamic mesh to the structural mesh, allowing their integration into the #gls("fem") analysis environment.
@fig_67 depicts a representative output from APAME, illustrating the distribution of lift coefficients, scaled by local chord length, along the wing span.
...
In @eq_79, $F_a$ indicates a point load, $rho$ denotes the density, $u^2$ signifies the velocity squared, $C_L$ is the lift coefficient, $c$ stands for the chord length, and $l$ defines the length.
The variable $q_a$ in @eq_80 characterizes a force distributed over length, commonly referred to as a distributed load.
...
To distinguish between the point load $F_a$ and the distributed load $q_a$, appropriate units are provided in the corresponding equations.
While in common language forces are often described as acting at specific points, this idealization rarely occurs in physical systems. In reality, forces typically manifest as distributed loads along lengths or as stresses (tension/pressure) over areas. 
...
A resultant force functions as a mathematical means of condensing a force distribution, represented by matrix $bold(F)^(n times m)$, into a single equivalent force. 
The matrix dimensions are defined by $n$, determining the number of force entries, and $m$, denoting the dimensionality of each force vector. 
The resultant can be expressed as either an $m$-dimensional vector preserving directional information or a scalar magnitude.

// ------------------------------- simple case ------------------------------ //
In simple cases, converting a resultant load distribution to a single load is possible without introducing concerning inaccuracies or local structural effects. Simple cases can be characterized by geometries that display minimal or no local gradients.
...
For example, if the local x and z coordinates exhibit minimal change while moving along the y direction of a beam, the corresponding geometric gradients are deemed low.
Beyond geometric considerations, structural analysis must account for material properties, including stiffness gradients and density distributions. 
These changes in material properties must be negligibly small or zero, ensuring constant material characteristics within the relevant domain. 
For the successful transformation of distributed loads to point loads without inducing local structural responses, both the shape and magnitude of the distributed load are of critical importance.


A distributed load can be transformed into a single point load over a specific length, with this transformation being most straightforward when the load maintains constant magnitude and angle. However, such transformations require careful evaluation of potential local structural responses. The risk of inducing these responses increases with both the non-uniformity of the load distribution and the length over which the transformation occurs.
For distributed loads acting over large lengths or areas, transformation to a single point load presents particular challenges. 
While mathematically feasible, the resulting concentrated force can induce significant local stresses, potentially leading to material failure at the point of application. This risk becomes especially pronounced when dealing with non-uniform load distributions.
...
As geometric complexity increases, the effective length over which a single resultant point load can adequately characterize the distributed load decreases, necessitating a higher number of discrete point loads to maintain structural accuracy.

// -------------------------------------------------------------------------- //
The meshed surface of a wing, serving as a complex geometry, is displayed on the left side of @fig_68. The figure demonstrates how the mesh consists of interconnected elements. This detailed representation is crucial for understanding the transformation of distributed loads to point loads.
The transformation process is motivated by two factors: APAME provides distributed loads according to @eq_80, while k-means++ @Arthur2006, as outlined in @chap_4_0, requires the aerodynamic load distribution as discrete data points for the initial step of #gls("ld").
Consequently, the distributed loads from @eq_80 must be converted to discrete point loads following the form of @eq_79.
The force transformation mechanism is visualized on the right side of @fig_68, where a single element exemplifies the relationship between the distributed force $q_(a,i)$ and its resulting point load $F_(a,i)$. 
...
The conversion of a uniform distributed load $q_a$ to a point load $F_a$ is accomplished through @eq_81, where $l$ represents the element's side length. 
Given that $q_a$ was derived according to @eq_80, which incorporates the chord-directional length, $l$ specifically denotes the element length in the span direction.

$ F_a = q_a dot l $ <eq_81>

// -------------------------------------------------------------------------- //
The aerodynamic mesh information is available in readable (non-binary) .vtk format, enabling straightforward processing. For handling .vtk and several other common file formats, such as XDMF, VTU, and GMSH @Geuzaine2009, several robust open-source libraries exist @Gueunet2023 @Shen2023 @Bethel2012. 
...
This calculation is accomplished through coordinate subtraction, where the coordinates of the right node are subtracted from those of the left node within the same element. Given the three-dimensional nature of the coordinates, the magnitude or length of the resulting vector is derived through the Pythagorean theorem, which can be mathematically expressed as the L2-norm shown in @eq_82. 
...
// ------------------------------- point load ------------------------------- //
The combination of @eq_81 and @eq_82 enables the transformation of distributed loads into point loads across all mesh elements, with each element yielding exactly one resultant partial force. However, as displayed in the lower left corner of @fig_69, this initial transformation does not provide the complete solution required for the current analysis.
...
The variation in force point density stems directly from the non-uniform element lengths within the mesh structure, as evident through careful inspection of @fig_67. 
...
K-means++ derives these positions through centroid calculation, providing the essential information about #gls("lie") centers. 
However, if the input data format inadvertently introduces directional bias, the generated k-means++ output may become partially or completely invalid for the intended analysis.
//
// ------------------------- input format adjustment ------------------------ //
The adjustment of k-means++ input data format necessitates establishing a relationship between force distribution and reference length. 
This relationship, which is based on each element's specific length, prescribes how many uniformly distributed partial point forces approximate the distributed load.
According to @eq_83, a distributed load [N $"mm"^(-1)$] spanning a reference length of $100 "mm"$ is discretized into 20 partial point loads [N]. 
These units, since chosen for clarity of exposition, remain adaptable to alternative requirements. 
Thus, @eq_83 provides the mathematical basis for determining the number of partial forces necessary to subdivide each resultant point load.
...
As formulated in @eq_86, the representative lift coefficient for a span strip $i$, denoted as $C_("lc",i)$, is derived through the integration of the local lift coefficient $C_L$ from the leading edge (le) to the trailing edge (te). 
...
Second, the reduced dimensionality allows for the use of one-dimensional beam models instead of the more computationally intensive #gls("fem") models within optimization frameworks.
A dedicated study of both the beam model and the optimization methodology is presented in @chap_4_2 to @chap_4_4, followed by concrete optimization problem definitions and their solutions in @chap_4_5_0 to @chap_4_5_3.
