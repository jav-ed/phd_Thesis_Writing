...
In @chap_3_4, an examination of available #glspl("lie", long:true) for introducing physical loads in the wing of a #gls("swith", long:true) was conducted. These elements were evaluated against specific criteria, leading to the establishment of the necessity of #gls("ld", long:true). 
The investigation continued in @chap_4_0, exploring various #gls("ld") possibilities and the insight that the elaborated methods require integration with optimization approaches to solve remaining issues. 
Prior to executing #gls("ld") and its subsequent steps, it is essential to obtain the aerodynamic loads acting on the aircraft wing. This section presents a methodology for acquiring these loads through numerical simulations and details their transformation from distributed loads to discrete point loads. The transformation process holds particular significance for k-means++ implementation and demonstrates advantageous implications for the subsequent optimization framework.

// -------------------------------------------------------------------------- //
// TODO FIpps is now open-source: https://github.com/TUD-LFT/FiPPS2
...
In @eq_79, $F_a$ indicates a point load, $rho$ denotes density, $u^2$ signifies velocity squared, $C_L$ is the lift coefficient, $c$ stands for chord length, and $l$ defines length. The variable $q_a$ in @eq_80 characterizes a force distributed over length, commonly referred to as a distributed load.
...
A resultant force serves as a mathematical means of condensing a force distribution, represented by matrix $bold(F)^(n times m)$, into a single equivalent force. 
...
The adjustment of k-means++ input data format necessitates establishing a relationship between force distribution and reference length. This relationship prescribes how many uniformly distributed partial point forces approximate the distributed load for each element, based on its specific length. According to @eq_83, a distributed load of [N $"mm"^(-1)$] spanning a reference length of $100 "mm"$ is discretized into 20 partial point loads [N]. 
...
A dedicated study of both the beam model and optimization methodology is presented in @chap_4_2 to @chap_4_4, followed by concrete optimization problem definitions and their solutions in @chap_4_5_0 to @chap_4_5_3.
